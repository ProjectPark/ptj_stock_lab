#!/usr/bin/env python3
"""
D2S v3 Regime Optuna 파라미터 최적화 — 레짐 감지 + 대규모 탐색
================================================================
backtest_d2s_v3.py (R19/R20/R21 레짐 조건부 청산) 전용.

v2 optimizer 대비 확장 (~45 파라미터):
  - R19: bb_entry_hard_max, bb_entry_hard_filter 탐색
  - R20: bull/bear 레짐별 take_profit 탐색
  - R21: bull/bear 레짐별 hold_days 탐색
  - 레짐 감지: bull/bear streak, SPY SMA, Polymarket BTC 임계값
  - 스코어 함수 변경: IS+OOS 동시 최적화 (과적합 억제)

스코어 함수: IS_Sharpe×10 + OOS_Sharpe×20 - |IS_MDD|×0.5
  (OOS 2배 가중 → 일반화 유도)

병렬 전략:
  JournalStorage(JournalFileBackend) — 20 workers 충돌 없음

Usage:
    # Stage 1: baseline 측정
    pyenv shell ptj_stock_lab && python simulation/optimizers/optimize_d2s_v3_regime_optuna.py --stage 1

    # Stage 2: Optuna 500 trials, 20 workers
    pyenv shell ptj_stock_lab && python simulation/optimizers/optimize_d2s_v3_regime_optuna.py \\
        --stage 2 --n-trials 500 --n-jobs 20

    # Stage 3: Full 검증
    pyenv shell ptj_stock_lab && python simulation/optimizers/optimize_d2s_v3_regime_optuna.py --stage 3
...
STUDY_NAME    = "d2s_v3_regime_optuna"
JOURNAL_PATH  = OPTUNA_DB_DIR / "d2s_v3_regime_journal.log"
BASELINE_JSON = RESULTS_DIR / "d2s_v3_regime_baseline.json"
DATA_PATH     = None   # None → market_daily.parquet (기본)
_BASE_PARAMS  = D2S_ENGINE_V3


def _apply_no_robn() -> None:
    """ROBN 제외 + 3y 데이터 + 2024-09-18 시작 모드로 전환.

    main() 호출 시 및 spawn worker 시작 시 호출.
    """
    ...
    OOS_END   = date(2026, 2, 17)
    STUDY_NAME    = "d2s_v3_regime_no_robn"
    JOURNAL_PATH  = OPTUNA_DB_DIR / "d2s_v3_regime_no_robn_journal.log"
    BASELINE_JSON = RESULTS_DIR / "d2s_v3_regime_no_robn_baseline.json"


# 모듈 로드 시점에 env var 확인 → spawn worker에서 자동 적용
...
def define_search_space(trial: optuna.Trial) -> dict:
    ...
    bull_tp = trial.suggest_float("bull_take_profit_pct",  4.0, 9.0, step=0.5)
    bear_tp = trial.suggest_float("bear_take_profit_pct",  3.0, 7.0, step=0.5)

    # ── [v3 신규] R21: 레짐 조건부 hold_days ──────────────
    bull_hd = trial.suggest_int("bull_hold_days_max",  5, 14)
    bear_hd = trial.suggest_int("bear_hold_days_max",  2, bull_hd)
    ...
    return {
        **_BASE_PARAMS,
        ...
        # v3: R21
        "bull_hold_days_max": bull_hd,
        "bear_hold_days_max": bear_hd,
    }
...
def calc_score(is_report: dict, oos_report: dict) -> float:
    ...
    score = IS_Sharpe×10 + OOS_Sharpe×20 - |IS_MDD|×0.5
...
def run_stage1() -> dict:
    print("\n" + "=" * 70)
    print("  [Stage 1] D2S v3 Regime Baseline — IS + OOS")
...
def run_stage2(
    ...
    print("\n" + "=" * 70)
    print(f"  [Stage 2] D2S v3 Regime Optuna — {n_trials} trials / {n_jobs} workers")
    ...
    if already_done == 0:
        ...
        study.enqueue_trial({
            ...
            "bull_take_profit_pct": p["bull_take_profit_pct"],
            "bear_take_profit_pct": p["bear_take_profit_pct"],
            "bull_hold_days_max": p["bull_hold_days_max"],
            "bear_hold_days_max": p["bear_hold_days_max"],
        })
        print("  v3 Regime 기본값 enqueue (warm start)")
...
def run_stage3(study_name: str = STUDY_NAME, journal_path: Path = JOURNAL_PATH) -> None:
    print("\n" + "=" * 70)
    print("  [Stage 3] D2S v3 Regime Full 검증")
    print("=" * 70)
...
def _save_optuna_report(
    ...
    lines = [
        ...
        f"> 탐색: v1+v2 전체 + R19(BB하드) + R20/R21(레짐 TP/HD) + Polymarket 레짐  ",
...
def main():
    parser = argparse.ArgumentParser(description="D2S v3 Regime Optuna 최적화")
    ...
    # no-robn 모드 적용 (env var 설정 → spawn worker에 전파)
    if args.no_robn:
    ...
    print("=" * 70)
    print("  D2S v3 Regime Optuna 최적화")
    print("  탐색: v1+v2 전체 + R19 + R20/R21(레짐) + Polymarket BTC 레짐")
    print(f"  IS: {IS_START} ~ {IS_END}  |  OOS: {OOS_START} ~ {OOS_END}")
    print(f"  스코어: IS_Shp×{SCORE_IS_SHARPE_W} + OOS_Shp×{SCORE_OOS_SHARPE_W} - MDD×{SCORE_MDD_W}")
    print("=" * 70)
