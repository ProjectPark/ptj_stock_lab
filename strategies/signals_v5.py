"""
PTJ 매매법 v5 - 시그널 엔진 (선별 매매형)
==========================================
v2 시그널을 기반으로 v5 변경사항 적용:
- 쌍둥이 ENTRY 갭: 1.5% → 2.2%
...
# ---------------------------------------------------------------------------
# 1. Market mode (v5: sideways 추가)
# ---------------------------------------------------------------------------

def determine_market_mode_v5(
...
# ---------------------------------------------------------------------------
# 3. Twin pairs (v5: entry threshold 변경)
# ---------------------------------------------------------------------------

def check_twin_pairs_v5(
    changes: dict,
    pairs: dict,
    entry_threshold: float = 2.2,
) -> list[dict]:
    """쌍둥이 페어 갭 분석 — v5: ENTRY 기준 2.2%.
...
def generate_all_signals_v5(
    ...
    *,
    ...
    # 1. 시황 판단 (횡보장 반영)
    market_mode = determine_market_mode_v5(poly_probs, sideways_active)
    ...
    # 3. 쌍둥이 페어 (v5: entry threshold 2.2%)
    twin_pairs = check_twin_pairs_v5(changes, pairs, entry_threshold=entry_threshold)

    # 4. 조건부 COIN (v5: trigger 4.5%)
