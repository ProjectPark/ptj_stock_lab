...
✅ P-3 ~ P-28 확정 반영본 (그대로 문서에 붙여도 됨)
P-3) M5 현금 차감(예약) 시점 — 개별 vs 세트
•	확정: 주문/종목 각각 따로 reserved_cash를 먼저 예약한다.
•	구현 메모:
o	주문 생성 시점에 reserved_cash = limit_price * qty + 예상수수료 즉시 차감(예약)
o	취소/만료/거절 시 reserved_cash 전액 반환
 
P-4) TSLL 200만원 한도 vs M5 비율 우선순위
•	확정: TSLL 200만원 한도가 우선 (예외 규정)
•	구현 메모:
o	M5가 TSLL에 99% 같은 큰 비중을 줘도 TSLL 매수금액은 2,000,000원으로 캡
o	캡으로 잘린 남는 돈은 현금 유지(같은 bar에서 재배분은 하지 않는 게 안전)
 
P-21) M4 “6일 과거데이터” 의미
•	확정: 매매 당일 기준으로 6일 전의 ‘60일선 상태’를 보라
•	구현 메모(안전한 해석):
o	t0 = today - 6 trading days
o	MA60(t0)를 계산하고, 그 기준으로 “60일선 위/아래” 판정
 
P-22) M80 순이익 “+0.1%” 기준
•	확정: 복잡한 +0.1% 해석 없음. 그냥 흑자(순이익 > 0)면 OK
•	구현 메모:
o	순이익이 0 이하 또는 결측이면 FAIL
 
P-23) ALL IN ONE “0.5% 미만” 관련 최종 확정
•	확정: 0.9% 이하에서는 ALL IN ONE 모드 자체가 절대 작동하지 않음
•	구현 메모(문장으로 고정):
o	“VNQ 하락폭(조건 산출값)이 **0.9% 이하(=약하면)**면 ALL IN ONE OFF”
o	즉, ALL IN ONE은 ‘충분히 큰 변동’일 때만 켠다.
 
P-24) 재난 이머전시 “30%+ 급변” 방향
•	확정: 상승/하락 방향 상관없이 둘 다 재난으로 본다.
•	구현 메모:
o	abs(p - p_prev) >= 0.30 (30%p 이상 변화)면 재난 트리거
 
P-25) M4에서 60일선 위 + 120일선 아래 동시일 때 우선순위
•	확정: 60일선부터 먼저 판단하고, 그 다음 120 기준을 본다.
•	구현 메모(충돌 방지용 결론):
o	평가 순서: 60MA 판정 → 120MA 판정
o	둘 다 걸리면 나중(120) 판정이 최종 상태를 덮어쓴다(“그 다음 120을 본다”를 코드로 안전하게 구현)
 
P-26) M4 “SHORT 중단”이 L전략 포함 여부
•	확정: “SHORT 중단”이면 숏모드만 중단하면 됨.
•	구현 메모:
o	신규 숏 진입만 금지
o	기존 숏 포지션의 청산/리스크관리(M200)는 계속 허용
 
P-27) A-2 시작가 3%~4% 구간 조정 포함 여부
•	확정: 구간 조정(1%→-1%, 2%→-2%, 3%→-3%) 같은 페널티는 포함하지 않음
•	구현 메모(깔끔한 규칙으로 고정):
o	시작가 조건은 “조정”이 아니라 허가/불허만 사용
o	(ALL IN ONE 문장과도 맞추려면) 3% 초과면 매수 금지, 3% 이하면 허가
(문서에 “3% 이하 허가”가 이미 있어서 이게 가장 자연스럽고 안전함)
 
P-28) BRKU 목표 +0.75%에 M20 가산 여부
•	확정: BRKU 목표를 0.75% → 1.0%로 정정하고,
M20 +0.25%를 더해서 최종 목표수익률을 1.25%로 고정한다.
•	문서 반영:
o	BRKU 목표(net) = 1.25% (최종)
P-1에서 ‘오류’가 나는 이유(핵심)
분배 규칙은 “1일 1번, 4종목을 순서대로 1주씩”인데 
taejun_strategy_review_2026-02-…
, 현실에선 아래 상황이 자주 생겨:
1.	수익금(또는 현금)이 부족해서 4종목을 다 못 삼
•	이때 어디까지 사고 멈추는지가 정의돼야 함(안 그러면 주문이 계속 실패/재시도, reserved_cash 꼬임, 현금 음수 같은 사고로 이어짐). 
•	SOXL 거래대금 ROBN 거래대금 GLD 거래대금 CONL거래대금
•	거래대금 많은 순으로 구매 
•	구매할 수 있는 현금 만큼 구매하기 현금이 부족해서 구매가 어려울경우 구매하지 않기
•	
taejun_strategy_review_2026-02-…
2.	동시 매수냐 / 순차 매수냐
•	동시에 4개 주문을 내면 “예약금(reserved_cash)”이 한 번에 크게 잡히면서 실패/거절이 늘고, 다른 전략(M5 매수)과 충돌할 수 있음.
•	순차면 “한 종목씩 확실히 사고 다음으로”라 안정적. 
•	순차적으로 동시에 주문을 하지않는다. 정정 
순자적으로 한종목씩 매수 다음 매수 다음 하는 식으로 순차적으로 하기 


taejun_strategy_review_2026-02-…
3.	실행 타이밍(하루에 몇 번 돌리냐)
•	매도할 때마다 즉시 분배하면 당일 다른 거래와 충돌하기 쉬움.
•	그래서 기본 설계는 장 마감 후 1회 실행이 안전함. 
•	장마감후 1회로 한다. 
taejun_strategy_review_2026-02-…
4.	가격 조회 실패/주문 거절 시 처리
•	어떤 종목 가격이 결측이거나 주문이 거절되면
“그 종목만 스킵하고 다음 종목으로 갈지 / 전체 중단할지”가 필요함. (이것도 P-1 성격의 ‘부족/실패 처리’에 포함)
•	주문거절시 그종목은 거절하고 다음 종목으로 해
•	실패사유는장마감후 3시간뒤에 MASTER H 현 MASTER J 준에게 M0시스템 알림 비고로 매수 실패 이유 알려줘 
•	✅ 정정 문장(가장 깔끔한 1안, 복붙용)
•	[P-1 정정: 수익금 분배 매수 순서]
수익금 분배 매수 순서는 고정 순서를 사용하지 않는다.
분배 실행 시점 기준, 대상 종목(SOXL/ROBN/GLD/CONL)의 **최근 거래세션 거래대금(=dollar volume)**을 조회하여 **거래대금이 큰 순서(내림차순)**로 정렬한 뒤, 1주씩 순차 매수한다.
거래대금 데이터가 없거나 동일한 경우에는 **기본 순서(SOXL→ROBN→GLD→CONL)**로 tie-break 한다
