...
from simulation.optimizers.optimize_v4_optuna import _get_baseline_params, _run_single_trial

PROJECT_ROOT = Path(__file__).resolve().parent
OUTPUT_JSON = PROJECT_ROOT / "data" / "v4_weight_experiments.json"
OUTPUT_MD = PROJECT_ROOT / "docs" / "v4_weight_experiments_report.md"

# Match the v4 Optuna search ranges so random experiments are comparable.
PARAM_SPACE = {
    "V4_PAIR_GAP_ENTRY_THRESHOLD": ("float", 1.0, 4.0, 0.2),
    "V4_DCA_MAX_COUNT": ("int", 1, 7, 1),
    "V4_MAX_PER_STOCK": ("int", 3_000, 10_000, 250),
    "V4_COIN_TRIGGER_PCT": ("float", 2.0, 7.0, 0.5),
    "V4_CONL_TRIGGER_PCT": ("float", 2.0, 7.0, 0.5),
    "V4_SPLIT_BUY_INTERVAL_MIN": ("int", 5, 30, 5),
    "V4_ENTRY_CUTOFF_HOUR": ("int", 10, 14, 1),
    "V4_ENTRY_CUTOFF_MINUTE": ("cat", [0, 30]),
    "V4_INITIAL_BUY": ("int", 1_000, 3_500, 250),
    "V4_DCA_BUY": ("int", 250, 1_500, 125),
    "V4_SIDEWAYS_MIN_SIGNALS": ("int", 2, 5, 1),
    "V4_SIDEWAYS_POLY_LOW": ("float", 0.30, 0.50, 0.05),
    "V4_SIDEWAYS_POLY_HIGH": ("float", 0.50, 0.70, 0.05),
    "V4_SIDEWAYS_GLD_THRESHOLD": ("float", 0.1, 0.8, 0.1),
    "V4_SIDEWAYS_INDEX_THRESHOLD": ("float", 0.2, 1.0, 0.1),
...
def _build_report(payload: dict, top_k: int) -> str:
    ...
    lines = [
        "# PTJ v4 Weight Experiment Report",
        "",
...
def run_experiments(n_trials: int, seed: int, top_k: int) -> dict:
    ...
    for trial_idx in range(1, n_trials + 1):
        ...
        print(
            f"[v4] trial {trial_idx:02d}/{n_trials:02d} "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run v4 weight experiments (baseline + random combinations)."
    )
    parser.add_argument(
        "--n-trials",
        type=int,
...
def main() -> None:
    ...
    print(f"[v4] JSON saved: {OUTPUT_JSON}")
    print(f"[v4] Report saved: {OUTPUT_MD}")
