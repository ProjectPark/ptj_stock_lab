#!/usr/bin/env python3
"""
PTJ v5 — Optuna 기반 파라미터 최적화
=====================================
Stage 1: 현재 config 가중치로 baseline 실행 → 리포트 저장
Stage 2: Optuna TPE sampler로 최적 파라미터 탐색 → 리포트 저장

Usage:
    pyenv shell market && python optimize_v5_optuna.py --stage 1
    pyenv shell market && python optimize_v5_optuna.py --stage 2 [--n-trials 20] [--n-jobs 6]
    pyenv shell market && python optimize_v5_optuna.py              # 1 → 2 연속 실행
"""
...
BASELINE_JSON = DATA_DIR / "v5_baseline_result.json"
BASELINE_REPORT = DOCS_DIR / "v5_baseline_report.md"
OPTUNA_REPORT = DOCS_DIR / "v5_optuna_report.md"
...
def _get_baseline_params() -> dict:
    ...
    return {
        # v5 고유
        "V5_PAIR_GAP_ENTRY_THRESHOLD": config.V5_PAIR_GAP_ENTRY_THRESHOLD,
        "V5_DCA_MAX_COUNT": config.V5_DCA_MAX_COUNT,
        "V5_MAX_PER_STOCK": config.V5_MAX_PER_STOCK,
        "V5_COIN_TRIGGER_PCT": config.V5_COIN_TRIGGER_PCT,
        "V5_CONL_TRIGGER_PCT": config.V5_CONL_TRIGGER_PCT,
        "V5_SPLIT_BUY_INTERVAL_MIN": config.V5_SPLIT_BUY_INTERVAL_MIN,
        "V5_ENTRY_CUTOFF_HOUR": config.V5_ENTRY_CUTOFF_HOUR,
        "V5_ENTRY_CUTOFF_MINUTE": config.V5_ENTRY_CUTOFF_MINUTE,
        "V5_INITIAL_BUY": config.V5_INITIAL_BUY,
        "V5_DCA_BUY": config.V5_DCA_BUY,
        # v5 횡보장
        "V5_SIDEWAYS_MIN_SIGNALS": config.V5_SIDEWAYS_MIN_SIGNALS,
        "V5_SIDEWAYS_POLY_LOW": config.V5_SIDEWAYS_POLY_LOW,
        "V5_SIDEWAYS_POLY_HIGH": config.V5_SIDEWAYS_POLY_HIGH,
        "V5_SIDEWAYS_GLD_THRESHOLD": config.V5_SIDEWAYS_GLD_THRESHOLD,
        "V5_SIDEWAYS_GAP_FAIL_COUNT": config.V5_SIDEWAYS_GAP_FAIL_COUNT,
        "V5_SIDEWAYS_TRIGGER_FAIL_COUNT": config.V5_SIDEWAYS_TRIGGER_FAIL_COUNT,
        "V5_SIDEWAYS_INDEX_THRESHOLD": config.V5_SIDEWAYS_INDEX_THRESHOLD,
        "V5_CB_GLD_SPIKE_PCT": config.V5_CB_GLD_SPIKE_PCT,
...
def _generate_baseline_report(result: dict, params: dict) -> str:
    ...
    v5_keys = [k for k in sorted(params.keys()) if k.startswith("V5_")]
    shared_keys = [k for k in sorted(params.keys()) if not k.startswith("V5_")]
    for k in v5_keys:
        v = params[k]
...
class BacktestObjectiveV5:
    """Optuna objective — v5 파라미터 탐색 공간 정의."""
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        params = {
            ...
            "V5_MAX_PER_STOCK": trial.suggest_int(
                "V5_MAX_PER_STOCK", 3_000, 10_000, step=250
            ),
            "V5_COIN_TRIGGER_PCT": trial.suggest_float(
                "V5_COIN_TRIGGER_PCT", 2.0, 7.0, step=0.5
            ),
            "V5_CONL_TRIGGER_PCT": trial.suggest_float(
                "V5_CONL_TRIGGER_PCT", 2.0, 7.0, step=0.5
            ),
            "V5_SPLIT_BUY_INTERVAL_MIN": trial.suggest_int(
                "V5_SPLIT_BUY_INTERVAL_MIN", 5, 30, step=5
            ),
            "V5_ENTRY_CUTOFF_HOUR": trial.suggest_int(
                "V5_ENTRY_CUTOFF_HOUR", 10, 14
            ),
            "V5_ENTRY_CUTOFF_MINUTE": trial.suggest_categorical(
                "V5_ENTRY_CUTOFF_MINUTE", [0, 30]
            ),
        ...
        # ---- v5 횡보장 ----
        params.update({
            "V5_SIDEWAYS_MIN_SIGNALS": trial.suggest_int(
                "V5_SIDEWAYS_MIN_SIGNALS", 2, 5
            ),
            "V5_SIDEWAYS_POLY_LOW": trial.suggest_float(
                "V5_SIDEWAYS_POLY_LOW", 0.30, 0.50, step=0.05
            ),
            "V5_SIDEWAYS_POLY_HIGH": trial.suggest_float(
                "V5_SIDEWAYS_POLY_HIGH", 0.50, 0.70, step=0.05
            ),
            "V5_SIDEWAYS_GLD_THRESHOLD": trial.suggest_float(
                "V5_SIDEWAYS_GLD_THRESHOLD", 0.1, 0.8, step=0.1
            ),
            "V5_SIDEWAYS_INDEX_THRESHOLD": trial.suggest_float(
                "V5_SIDEWAYS_INDEX_THRESHOLD", 0.2, 1.0, step=0.1
            ),
            "V5_CB_GLD_SPIKE_PCT": trial.suggest_float(
                "V5_CB_GLD_SPIKE_PCT", 2.0, 5.0, step=0.5
            ),
            "V5_CB_GLD_COOLDOWN_DAYS": trial.suggest_int(
                "V5_CB_GLD_COOLDOWN_DAYS", 1, 5
            ),
            "V5_CB_BTC_CRASH_PCT": trial.suggest_float(
                "V5_CB_BTC_CRASH_PCT", -8.0, -3.0, step=0.5
            ),
            "V5_CB_BTC_SURGE_PCT": trial.suggest_float(
                "V5_CB_BTC_SURGE_PCT", 3.0, 8.0, step=0.5
            ),
        })
...
def run_stage2(
    ...
    if n_jobs > 1:
        ...
        with ctx.Pool(processes=n_jobs) as pool:
            objective = BacktestObjectiveV5(pool=pool, gap_max=gap_max)
            study.optimize(
                objective,
                n_trials=n_trials,
                timeout=timeout,
        ...
        objective = BacktestObjectiveV5(pool=None, gap_max=gap_max)
        study.optimize(
            objective,
            n_trials=n_trials,
            timeout=timeout,
...
def main():
    ...
    parser.add_argument("--gap-max", type=float, default=4.0, help="V5_PAIR_GAP_ENTRY_THRESHOLD 탐색 상한 (%%, 기본: 4.0)")
