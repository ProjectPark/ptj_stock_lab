...
from .filters.filters import GoldFilter, MarketModeFilter, SidewaysDetector
from .strategies.soxl_independent import SoxlIndependentStrategy
from .filters.stop_loss import StopLossCalculator
from .filters.swing_mode import SwingModeManager
from .strategies.twin_pair import TwinPairStrategy
from .strategies.conditional_coin import ConditionalCoinStrategy
from .strategies.conditional_conl import ConditionalConlStrategy
from .strategies.bearish_defense import BearishDefenseStrategy
from .infra.m201_mode import M201ImmediateMode
from .strategies.vix_gold import VixGold

if TYPE_CHECKING:
    from .common.base import MarketData, Position
...
class CompositeSignalEngine:
    ...
    def __init__(
        ...
        # M201 즉시전환 모듈
        self._m201: M201ImmediateMode | None = None
    ...
    def _prepare_session_and_filters(
        ...
        # M201 즉시전환 체크 (M200 다음 우선순위)
        m201_result = None
        if hasattr(self, '_m201') and self._m201 is not None:
            if market_data and market_data.poly and market_data.poly_prev:
                btc_p = market_data.poly.get("btc_up", 0.5)
                btc_p_prev = market_data.poly_prev.get("btc_up", 0.5)
                m201_check = self._m201.check_long(btc_p, btc_p_prev)
                if m201_check is None:
                    m201_check = self._m201.check_short(btc_p, btc_p_prev)
                m201_result = m201_check
