...
Usage:
    pyenv shell market && python optimize_v3_optuna.py --stage 1
    pyenv shell market && python optimize_v3_optuna.py --stage 2 [--n-trials 20] [--n-jobs 6]
    pyenv shell market && python optimize_v3_optuna.py              # 1 → 2 연속 실행
"""
...
BASELINE_JSON = DATA_DIR / "v3_baseline_result.json"
BASELINE_REPORT = DOCS_DIR / "v3_baseline_report.md"
OPTUNA_REPORT = DOCS_DIR / "v3_optuna_report.md"
...
def _generate_baseline_report(result: dict, params: dict) -> str:
    ...
    v3_keys = [k for k in sorted(params.keys()) if k.startswith("V3_")]
    shared_keys = [k for k in sorted(params.keys()) if not k.startswith("V3_")]
    for k in v3_keys:
        v = params[k]
    ...
    def __call__(self, trial: optuna.Trial) -> float:
        ...
        params = {
            ...
            "V3_COIN_TRIGGER_PCT": trial.suggest_float(
                "V3_COIN_TRIGGER_PCT", 4.5, 6.5, step=0.5
            ),
            "V3_CONL_TRIGGER_PCT": trial.suggest_float(
                "V3_CONL_TRIGGER_PCT", 5.5, 7.5, step=0.5
            ),
        ...
        params.update({
            "V3_SIDEWAYS_MIN_SIGNALS": trial.suggest_int(
                "V3_SIDEWAYS_MIN_SIGNALS", 2, 3
            ),
            "V3_SIDEWAYS_POLY_LOW": trial.suggest_float(
                "V3_SIDEWAYS_POLY_LOW", 0.30, 0.40, step=0.05
            ),
            "V3_SIDEWAYS_POLY_HIGH": trial.suggest_float(
                "V3_SIDEWAYS_POLY_HIGH", 0.45, 0.55, step=0.05
            ),
            "V3_SIDEWAYS_GLD_THRESHOLD": trial.suggest_float(
                "V3_SIDEWAYS_GLD_THRESHOLD", 0.2, 0.5, step=0.1
            ),
            "V3_SIDEWAYS_INDEX_THRESHOLD": trial.suggest_float(
                "V3_SIDEWAYS_INDEX_THRESHOLD", 0.8, 1.0, step=0.1
            ),
        })
...
def run_stage2(
    ...
    objective = BacktestObjectiveV3(gap_max=gap_max)
    ...
    # Parallel optimization using Optuna's built-in parallelization
    # Run in multiple processes manually
    import subprocess
    import sys

    if n_jobs > 1:
        print(f"  병렬 실행: {n_jobs} 프로세스 수동 실행")
        print(f"  각 프로세스가 DB를 공유하며 자동으로 작업을 분배합니다.")
        print()

        # 각 worker는 동일한 명령을 실행하되, Optuna가 DB를 통해 작업 분배
        processes = []
        for i in range(n_jobs):
            # 첫 번째 프로세스는 현재 프로세스 사용
            if i == 0:
                continue
            cmd = [
                sys.executable,
                ...
                f"""
import optuna
from optimize_v3_optuna import BacktestObjectiveV3
study = optuna.load_study(study_name='{study_name}', storage='{storage}')
objective = BacktestObjectiveV3(gap_max={gap_max})
study.optimize(objective, n_trials={n_trials // n_jobs + 1}, timeout={timeout}, show_progress_bar=False)
"""
            ]
...
def main():
    ...
    parser.add_argument("--gap-max", type=float, default=4.0, help="V3_PAIR_GAP_ENTRY_THRESHOLD 탐색 상한 (%%, 기본: 4.0)")
